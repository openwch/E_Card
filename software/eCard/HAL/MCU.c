...
tmosEvents HAL_ProcessEvent(tmosTaskID task_id, tmosEvents events)
{
    ...
    if(events & HAL_BATTERY_EVENT)
    {
        if (batteryMonitorFlag)
        {
            ...
            {
                batCount = 0;
                g_BattVal = (batteryRead()-320)&0XFF;
                if(g_BattVal != battVal)
                {
                    battVal = g_BattVal&0xff;
                    LCD_DrawBattVal(battVal);
                }
            }
        }
        return events ^ HAL_BATTERY_EVENT;
    }
    ...
    if(events & HAL_TIME_EVENT)
    {
        if((hour != calendar.hour)||(min != calendar.min))
        {
            hour = calendar.hour;
            min  = calendar.min;
            LCD_DrawTime(hour,min);
        }

        if((w_year != calendar.w_year)||(w_month != calendar.w_month)||(w_date != calendar.w_date))
        {
            w_year = calendar.w_year;
            w_month = calendar.w_month;
            w_date = calendar.w_date;
            LCD_DrawDate(w_year,w_month,w_date);
        }

        if ((!screenTimeout)&&(g_BattVal<30))//电量低于30%开始息屏
        {
            screenTimeoutCount++;
            if (screenTimeoutCount > 18000) //无操作5分钟息屏30000
            {
                screenTimeoutCount = 0;
                screenTimeout = ENABLE;
                LCD_SetBackLight(BRIGHT_MIN);
            }
        }
...
void HAL_Init()
{
    ...
    // HAL_KeyInit();
    HAL_TouchKeyInit();
    Hal_TouchKeyConfig(LCD_DisplayPage_Switch);
    HAL_GetBatteryVal();
#endif
#if (defined BLE_CALIBRATION_ENABLE) && (BLE_CALIBRATION_ENABLE == TRUE)
    tmos_start_task(halTaskID, HAL_REG_INIT_EVENT, MS1_TO_SYSTEM_TIME(BLE_CALIBRATION_PERIOD)); // 添加校准任务，单次校准耗时小于10ms
#endif
    tmos_start_reload_task(halTaskID, HAL_TIME_EVENT, MS1_TO_SYSTEM_TIME(10));
    tmos_start_reload_task(halTaskID, HAL_BATTERY_EVENT, MS1_TO_SYSTEM_TIME(1000)); /* Kick off polling */
    //    tmos_start_task(halTaskID, HAL_TEST_EVENT, MS1_TO_SYSTEM_TIME(1000));    // 添加一个测试任务
}

// void HAL_TKY_BAT_Init(void)
// {
//     HAL_TouchKeyInit();
//     Hal_TouchKeyConfig(LCD_DisplayPage_Switch);
//     HAL_GetBatteryVal();
