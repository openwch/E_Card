...
uint8_t uart_line_sta_buf[2] = {0,0};
...
//类请求获取波特率参数
uint8_t *Class_GetLineCoding(uint16_t Length)
{
    //这个是获取描述符的处理
//    uint32_t  wOffset;
//
...
//    return pDesc->Descriptor + wOffset;
    //先这样写
    //传输长度
    pInformation->Ctrl_Info.Usb_wLength = sizeof(LINE_CODE);
    //返回传输数据指针
    return (uint8_t *)&Uart0Para;
}


//类请求获取参数
uint8_t *Class_GetCtrlLineSta(uint16_t Length)
{
    //这个是获取描述符的处理
//    uint32_t  wOffset;
//
...
//    return pDesc->Descriptor + wOffset;
    //传输长度
    pInformation->Ctrl_Info.Usb_wLength = sizeof(uart_line_sta_buf);

    return uart_line_sta_buf;
}
...
//#define Type_Recipient (pInformation->USBbmRequestType & (REQUEST_TYPE | RECIPIENT))
  //其他类请求处理 （无数据阶段）
  else if(RequestNo == DEF_SET_CONTROL_LINE_STATE)  //设置线路状态
  {
      Result = USB_SUCCESS;  //不处理直接返回成功
  }
...
//  pInformation->ControlState = SETTING_UP;
  ...
  if (Request_No == GET_DESCRIPTOR)    //获取描述符
  {
    if (Type_Recipient == (STANDARD_REQUEST | DEVICE_RECIPIENT))
    {
      ...
      if (wValue1 == DEVICE_DESCRIPTOR)  //str1
      {
          //pProperty->GetDeviceDescriptor运行的函数
//          uint8_t *Standard_GetDescriptorData(uint16_t Length, ONE_DESCRIPTOR *pDesc)
        CopyRoutine = pProperty->GetDeviceDescriptor;
      }
      else if (wValue1 == CONFIG_DESCRIPTOR)  //str2
      {
        CopyRoutine = pProperty->GetConfigDescriptor;
      }
      else if (wValue1 == STRING_DESCRIPTOR)  //str3
      {
        CopyRoutine = pProperty->GetStringDescriptor;
           ...
           && (pInformation->USBwIndex1 == 0))   //获取状态
  {
  ...
  else if (Request_No == GET_CONFIGURATION)   //获取配置
  {
    if (Type_Recipient == (STANDARD_REQUEST | DEVICE_RECIPIENT))
    {
  ...
  else if (Request_No == GET_INTERFACE)    //获取接口
  {
  ...
  //看一程序的处理流程将下面进行分类
  //类请求码处理  数据上传处理
  else if(Request_No == DEF_GET_LINE_CODING)  //获取波特率参数
  {
      CopyRoutine = Class_GetLineCoding;
  }
  else if(Request_No == DEF_SERIAL_STATE)     //获取线路状态
  {
      CopyRoutine = Class_GetCtrlLineSta;
  }
  //类请求码处理  数据下发处理
  else if(Request_No == DEF_SET_LINE_CODING)  //设置波特率
  {
      //CopyRoutine = Uart0Para;
  }
  //这个在无数据阶段处理
//  else if(Request_No == DEF_SET_CONTROL_LINE_STATE)  //设置线路状态
//  {
//
//
//  }
  ...
  if (ValBit(pInformation->USBbmRequestType, 7))  //数据上传处理
  {
